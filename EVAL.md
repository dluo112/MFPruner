...
We evaluate MFPruner with different [LLaVA](https://github.com/haotian-liu/LLaVA) models on a diverse set of 10 benchmarks. To ensure the reproducibility, we evaluate the models with greedy decoding following the originial LLaVA.
...
To evaluate MFPruner with LLaVA-NeXT, you just need to replace the `v1_5` with `v1_6` in the shell scripts. For example, to evaluate VQAv2 with LLaVA-NeXT, you can run:
