...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        q, k = apply_rotary_pos_emb_vision(q, k, cos, sin)
        attention_mask = torch.full(
            [1, seq_length, seq_length], True, device=q.device, dtype=torch.bool
        )
        for i in range(1, len(cu_seqlens)):
        ...
        attn_output_sdpa = F.scaled_dot_product_attention(
            q_sdpa, k_sdpa, v_sdpa, attn_mask=attention_mask, dropout_p=0.0
        ) 
        attn_output = attn_output_sdpa.transpose(1, 2).squeeze(0).reshape(seq_length, -1)
        attn_output = self.proj(attn_output)

        if return_logits:
            return_k = k.permute(1, 0, 2) 
            q_manual = q.permute(1, 0, 2) 
            k_manual = k.permute(1, 0, 2)
            attn_weights = torch.matmul(q_manual, k_manual.transpose(1, 2)) / math.sqrt(self.head_dim)
            
            float_mask = torch.where(
                attention_mask,
                torch.finfo(attn_weights.dtype).min,
                0.0 
            ).to(attn_weights.dtype).squeeze(0)
            attn_weights = attn_weights + float_mask
            attn_logits = F.softmax(attn_weights, dim=-1, dtype=torch.float32).to(q.dtype)
            return attn_output, attn_logits, return_k
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        return hidden_states, attn_mean, attn_key, window_index, cu_window_seqlens
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_embeds, attn_logits, attn_key, window_index, cu_window_seqlens = self.visual(pixel_values, grid_thw=image_grid_thw)
                image_embeds_for_window = image_embeds.clone().detach()
        ...
        if select_pixel:
            ...
            image_token_indices = torch.where(input_ids.squeeze(0) == self.config.image_token_id)[0].to(device)
            N_img = image_embeds_for_window.shape[0]

            if attn_logits.ndim > 1:
                attn_logits = attn_logits.flatten()
            attn_logits = attn_logits.to(device)
            ...
            # attention cue
            metric1 = attn_logits  # [N_img]
            topk_indices_1 = torch.topk(metric1, k).indices.view(-1)
            
            # similarity cue
            window_similarity_full = self.compute_token_similarity(image_embeds_for_window.to(device),window_index,cu_window_seqlens)
            metric2 = 1.0 - window_similarity_full  
            topk_indices_2 = torch.topk(metric2, k).indices.view(-1)

            # text-relevance cue
            text_mask = (input_ids != self.config.image_token_id).squeeze(0)
            text_embeddings = inputs_embeds[0, text_mask, :]
            
            if text_embeddings.numel() == 0:
                txt_mean = torch.zeros((1, D), device=device, dtype=inputs_embeds.dtype)
            else:
                txt_mean = text_embeddings.mean(dim=0, keepdim=True)

            img_norm = image_embeds_for_window.to(device) / (image_embeds_for_window.norm(dim=-1, keepdim=True).to(device) + 1e-12)
            txt_norm = txt_mean / (txt_mean.norm(dim=-1, keepdim=True) + 1e-12)
            metric3 = torch.matmul(img_norm, txt_norm.transpose(0, 1)).squeeze(-1)
            topk_indices_3 = torch.topk(metric3, k).indices.view(-1)
            
            del text_embeddings, txt_mean, img_norm, txt_norm, window_similarity_full

            # vote scheme
            all_indices = torch.cat([topk_indices_1, topk_indices_2], dim=0)
            votes = torch.bincount(all_indices, minlength=N_img)
            dominant_mask = votes >= 2
            dominant_indices = torch.where(dominant_mask)[0]
            
            if dominant_indices.numel() < k:
                combined_metric = (metric3).view(-1)
                num_tokens = combined_metric.numel()
                mask = torch.ones(num_tokens, dtype=torch.bool, device=combined_metric.device)
                if dominant_indices.numel() > 0:
                    mask[dominant_indices] = False
                _, sorted_idx = torch.sort(combined_metric[mask], descending=True)
                all_indices = torch.arange(num_tokens, device=combined_metric.device)
                remaining_candidates = all_indices[mask][sorted_idx]
                need = k - dominant_indices.numel()
                supplement = remaining_candidates[:need]
                final_indices = torch.cat([dominant_indices, supplement], dim=0)
                final_indices = torch.unique(final_indices)
                if final_indices.numel() < k:
                    _, extra_idx = torch.topk(combined_metric, k)
                    final_indices = torch.unique(torch.cat([final_indices, extra_idx], dim=0))[:k] 
            else:
                final_indices = torch.topk(votes, k).indices.view(-1)

            final_indices = final_indices.to(device).view(-1)

            img_mask = torch.zeros(L, dtype=torch.bool, device=device)
            selected_global_indices = image_token_indices[final_indices]
            img_mask[selected_global_indices] = True
            non_image_mask = (input_ids.squeeze(0) != self.config.image_token_id)
            img_mask = img_mask | non_image_mask
    ...
    def compute_token_similarity(self, image_features):
            """向量化计算每个token与其他token的相似度的平均值"""
            tokens = image_features  # [576, 4096]
            
            # 归一化token向量(余弦相似度就是归一化后的点积)
