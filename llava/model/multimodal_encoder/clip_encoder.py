...
class CLIPVisionTower(nn.Module):
    ...
    def load_text_tower(self, device_map=None):
        vision_tower_with_projection = CLIPVisionModelWithProjection.from_pretrained(self.vision_tower_name).to("cuda")
        ...
        self.text_tower = CLIPTextModelWithProjection.from_pretrained(self.vision_tower_name).to("cuda")
    ...
    def forward(self, images, texts=None, output_attentions=False):
        if type(images) is list:
            ...
            with torch.cuda.stream(image_stream):
                ...
                if output_attentions:
                    image_features = (
                        image_outputs[0].to(images.dtype), #image features(1×576×1024)
                        image_outputs[1].to(images.dtype), #image attentions(1×16×576)
            ...
            if texts is not None:
                image_embeds = self.vision_tower.vision_model.post_layernorm(image_outputs[0])
